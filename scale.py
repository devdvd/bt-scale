...
def start_measurement_script(scale_mac_address=None, measure_alias=None):
    ...
    def user(data):
        ...
        _name = (
            (chr(_name_c1) if _name_c1 > 0 else '') +
            (chr(_name_c2) if _name_c2 > 0 else '') +
            (chr(_name_c3) if _name_c3 > 0 else '')
        ).strip()
        if VERBOSE:
            print(' - name', _name)
    ...
    def compile_user(user):
        ...
        _name = user['name']
        _name_0 = '{:02x}'.format(ord(_name[0])) if len(_name) > 0 else '00'
        _name_1 = '{:02x}'.format(ord(_name[1])) if len(_name) > 1 else '00'
        _name_2 = '{:02x}'.format(ord(_name[2])) if len(_name) > 2 else '00'
    ...
    def read_line(child, token='0x002e value: ', timeout=4, include_active=False):
        """
        Reads a line from the GATT tool output and processes it based on the given token.
        If include_active is False, lines starting with 'e7 58' (indicating active measurement)
        will be ignored.
        """
        line = ''
        try:
            child.expect(token, timeout=timeout)
            line = child.readline().strip().decode('utf-8')  # Decode binary output to string
        ...
        if include_active:
            ...
            while True:
                if line.startswith('e7 58'):
                    try:
                        if VERBOSE_COMM:
                            print('ignoring active measurement', line, '->', weight(line)['weight'])
                        child.expect(token, timeout=4)
    ...
    def check_status(status, message):
        """
        Checks the status of an operation. If the status is False, prints an error message
        and exits the program.
        """
        if not status:
            print('\nUnable to get ' + message + ' status, will exit now. (TODO: implement retry)\n')
            sys.exit(1)
    ...
    # Starte den gatttool-Prozess und verbinde
    child = pexpect.spawn(f'gatttool -I -b {scale_mac_address}')

    # Falls LOG_PEXPECT aktiviert ist, öffne das Logfile
    if LOG_PEXPECT:
        log_file = open("pexpect.log.txt", "w")
    ...
    try:
        # Hier kannst du beliebige Aktionen ausführen, z.B. das Abwarten einer Antwort:
        child.expect('Connection successful')  # Beispiel, du kannst das auf die erwartete Antwort anpassen
        print("Connection established!")
        
        # Weitere Logik hier
        messages.append("Connected successfully")
        
    except pexpect.TIMEOUT:
        print("Connection timed out.")
        messages.append("Connection timed out")
    except pexpect.ExceptionPexpect as e:
        print(f"Error: {str(e)}")
        messages.append(f"Error: {str(e)}")
    finally:
        # Schließe die Log-Datei, wenn sie geöffnet wurde
        if LOG_PEXPECT:
            log_file.close()

        # Weitere Verarbeitung oder Ausgabe der Nachrichten
    ...
    # Öffne das Logfile und initialisiere den DualLogger
    with open("pexpect.log.txt", "wb") as log_file:
        child = pexpect.spawn(f'gatttool -I -b {scale_mac_address}')
    ...
    ###################################################
        ...
        if VERBOSE_COMM: 
            print('')
            print('---------------------------------------------')
            print(f'Get user details for {user["name"]}')
        ...
        if VERBOSE_COMM: 
            print(f'User detail status -> {user_detail_status}')
        
        user_detail_status = user_detail(user_detail_status)
        user_detail_status['uid'] = _current_user_id
        if VERBOSE_COMM: 
            print(json.dumps(user_detail_status, indent=2))
        ...
        if len(users_in_scale_but_not_in_file):
            ...
            for user in users_in_scale_but_not_in_file:
                ...
                if answer == 'YES':
                    ...
                    print('')
                    exit_data = {'sys-exit': 'user-deleted-please-review'}
                    return exit_data
                else:
                    print('')
                    ...
                    print('')
                    exit_data = {'sys-exit': 'user-not-deleted-please-review'}
                    return exit_data
    ...
    ###################################################
        ...
        if len(users_in_file_but_not_in_scale):
            ...
            for user in users_in_file_but_not_in_scale:
                ...
                print('Will exit now so that you can think about it.')
                print('')
                exit_data = {'sys-exit': 'user-added-please-review'}
                return exit_data
            ...
            for user_id in users_in_file_and_in_scale:
                ...
                if data_in_file == data_in_scale:
                    ...
                    if answer == 'YES':
                        ...
                        print('Will exit now so that you can think about it.')
                        print('')
                        exit_data = {'sys-exit': 'user-updated-please-review'}
                        return exit_data
                    else:
                        print('')
                        ...
                        print('')
                        exit_data = {'sys-exit': 'user-differs-please-review'}
                        return exit_data
