...
def scale(data):
    ...
    if VERBOSE:
        print(' - weight threshold', _weight)

    offset = 6
    _fat = int(data[offset:offset+2], 16) / 10.0
    if VERBOSE:
        print(' - fat threshold', _fat)

    offset = 8
    _unit = int(data[offset:offset+2], 16)
    if VERBOSE:
        print(' - unit', _unit)  # 1=kg, 2=lb, 4=st
...
def user_list(data):
    ...
    if VERBOSE:
        print(' - user count', _users_count)
    
    offset = 4
    _users_max = int(data[offset:offset+2], 16)
    if VERBOSE:
        print(' - max users', _users_max)
...
def user(data):
    ...
    if VERBOSE:
        print(' - user index', _user_index)
    offset = 2
    _users_count = int(data[offset:offset+2], 16)
    if VERBOSE:
        print(' - max users', _users_count)
    ...
    if VERBOSE:
        print(' - user id hex', _user_id)

    offset = 20
    _name_c1 = int(data[offset+0:offset+0+2], 16)
    _name_c2 = int(data[offset+2:offset+2+2], 16)
    _name_c3 = int(data[offset+4:offset+4+2], 16)
    _name = ((chr(_name_c1) if _name_c1 > 0 else '') +
             (chr(_name_c2) if _name_c2 > 0 else '') +
             (chr(_name_c3) if _name_c3 > 0 else '')).strip()
    if VERBOSE:
        print(' - name', _name)
...
# Assuming VERBOSE is defined elsewhere
...
def check_status(status, message):
    if not status:
        print('')
        print(f'unable to get {message} status, will exit now. (TODO: implement retry)')
        print('')
        sys.exit(1)
...
child = pexpect.spawn(f'gatttool -I -b {scale_mac_address}')
if LOG_PEXPECT:
    child.logfile = open("pexpect.log.txt", "w")
child.sendline('connect')
try:
    
    # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    child.expect(f'Attempting to connect to {scale_mac_address}', timeout=4)
    print('connecting...')
    ...
    cmd = WRITE_REQ + f'E9 {readout["timestamp"]:X}'.replace(' ', '')
    if VERBOSE_COMM: print(cmd)
    child.sendline(cmd)
    ...
    if VERBOSE_COMM: print(f'scale status -> {scale_status}')
    ...
    if VERBOSE_COMM: print(f'userlist status -> {userlist_status}')
    ...
    if userlist_count > 0:
        ...
        for i in range(userlist_count):
            ...
            if VERBOSE_COMM: print(f'user status -> {user_status}')
            ...
            cmd = WRITE_REQ + f'E7 F1 34 {userlist_count:02x} {(i+1):02x}'.replace(' ', '')
            if VERBOSE_COMM: print(cmd)
            child.sendline(cmd)
...
for user in readout['users']:
    ...
    if VERBOSE_COMM: print('')
    if VERBOSE_COMM: print('---------------------------------------------')
    if VERBOSE_COMM: print(f'get user details for {user["name"]}')
    ...
    if VERBOSE_COMM: print(f'user detail status -> {user_detail_status}')
...
if len(users_in_scale_but_not_in_file):
    ...
    for user in users_in_scale_but_not_in_file:
        ...
        print('')
        answer = input("Type 'YES' (exactly as shown) to DELETE the user\nfrom the scale or press ENTER to leave it there: ")
        if answer == 'YES':
            ###################################################
            # DELETE USER
            ###################################################
...
if len(users_in_file_and_in_scale):
    ...
    for user_id in users_in_file_and_in_scale:
        ...
        if data_in_file == data_in_scale:
            print(f'user details in file match those in the scale for uid {user_id}')
        else:
            print('')
            print(f'USER DETAILS IN FILE DIFFER FROM THOSE IN SCALE FOR UID {user_id}')
            print('')
            print(f' -  file: {data_in_file}')
            print(f' - scale: {data_in_scale}')
            print('')
            print('DO YOU WANT TO SAVE DETAILS FROM THE FILE TO THE SCALE?')
            print('')
            answer = input("Type 'YES' (exactly as shown) to UPDATE the user\nfrom the scale or press ENTER to leave it there: ")
            if answer == 'YES':
...
if DO_CHECK_UNKNOWN:
    ...
    if VERBOSE_COMM: print(f'measurements_status: {measurements_status}')
    ...
    if measurements_status == 'e7 f0 46 00':  # scale responds with 'e7 f0 46 01' if there are none
        ...
        while True:
            ...
            if VERBOSE_COMM: print(f'unknown_measurement_status: {unknown_measurement_status}')
        ...
        if DO_SAVE_UNKNOWN:
            ...
            with open(f'unknown-measurements-{_time}.json', 'w') as f:
                json.dump(unknown_measurements, f, indent=2)

        if DO_DELETE_UNKNOWN:
            for unknown_measurement in unknown_measurements:
                if VERBOSE_COMM: print(f'delete unknown measurement from slot {unknown_measurement["slot"]}')
                ...
                if VERBOSE_COMM: print(f'delete_unknown_measurement_status: {delete_unknown_measurement_status}')
...
for user in readout['users']:
    ...
    if VERBOSE_COMM: print('')
    if VERBOSE_COMM: print('---------------------------------------------')
    if VERBOSE_COMM: print(f'request saved measurements for {user["name"]}')
    ...
    if VERBOSE_COMM: print(f'measurements status -> {measurements_status} -> {measurements_count / 2} stored measurements')
    if measurements_count > 0:
        ...
        for i in range(measurements_count):
            ...
            if VERBOSE_COMM: print(f'measurement {i+1}: {measurement_status}')
            if i % 2 == 0:
            ...
            if i % 2 == 1:
                ...
                data = []
            cmd = WRITE_REQ + f'E7 F1 42 {measurements_count:02X} {i + 1:02X}'.replace(' ', '')
            if VERBOSE_COMM: print(cmd)
            child.sendline(cmd)

        if DO_DELETE_MEASUREMENTS:
            if VERBOSE_COMM: print(f'delete saved measurements for {user["name"]}')
...
if DO_MEASURE:
    ...
    if not _measure_user_id or _measure_user_id not in users_in_file_and_in_scale:
        print('')
        print(f'EXITING, USER {measure_alias} IS NOT PROPERLY SET UP, SO NO MEASUREMENTS WILL BE DONE')
        print('')
        return {'sys-exit': 'user-not-set-up-please-review'}

    messages.append(f'measuring begins after {round(time.time() - readout["timestamp"], 3)} seconds')

    if VERBOSE_COMM: print('')
    if VERBOSE_COMM: print('---------------------------------------------')
    if VERBOSE_COMM: print(f'do a measurement for {measure_alias}')
    ...
    while True:
        try:
            ...
            if not measurement_status:
                ...
                print(f'ACTIVE {measurement_status} -> {_weight["weight"]}')
                ...
                _weight = weight(measurement_status)
                print(f'FINAL {measurement_status} -> {_weight["weight"]}')
                ...
                # Ack part 3/3 of summary
                if VERBOSE_COMM: print('ack part 3/3 of summary')
                cmd = WRITE_REQ + 'E7 F1 59 03 03'.replace(' ', '')
                if VERBOSE_COMM: print(cmd)
                child.sendline(cmd)
                if DO_DELETE_MEASUREMENTS:
                    if VERBOSE_COMM: print(f'delete saved measurements for {measure_alias}')
...
# DONE, MAYBE CHECK IF THERE IS STILL SOME DATA IN THE AIR
if False:
    if VERBOSE_COMM: print('')
    if VERBOSE_COMM: print('---------------------------------------------')
    if VERBOSE_COMM: print('something else?')
    something_status = read_line(child, token="0x002e value: ", timeout=3)
    if not something_status:
        print('nope.')
    else:
        print(f'yes {something_status}')
...
if not aborting and 'timestamp' in readout:
    ...
    print('')
    print(f'all done in script.py in {round(time.time() - readout["timestamp"], 3)} seconds')
    print('')
